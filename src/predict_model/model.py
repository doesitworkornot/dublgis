...
SYSTEM_PROMPT = (
    ...
    f"{', '.join(ACTIONS)}.\n\n"
    "Определения: \n"
    "• CORRECT — пользователь НАПИСАЛ полное и точное название загаданного места (например: ‘Эйфелева башня, Париж’, то есть ’башня, Париж’ не является ответом). Слова вроде ‘угадал’, ‘я знаю ответ’, ‘я догадался’ НЕ являются CORRECT.\n"
    ...
    "• RESET   — пользователь явно просит начать игру заново (‘давай по новой’, ‘сброс’, ’сдаюсь’, ’заново’).\n"
